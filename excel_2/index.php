...
<div class="container py-5">
  ...
  <h5>Target Date: </h5>
      ...
      <form method="post" action="">
        <div class="form-group row">
        <?php 
          $select  = "select * from date_table where id= 1";
         $run = mysqli_query($con,$select);
          while($rows = mysqli_fetch_array($run)){ ?>
             <input type="text" name="date" class="form-control py-1" value="<?php echo $rows['date'];?>"/>
             <?php }?>
			  <input type="submit" name="datesuvmit" class="btn btn-primary mt-2 " value="Submit">
      </form>
  </div>
    </div>
  </div>
...
<?php
  ...
  if(isset($_POST['datesuvmit'])){

    $date = $_POST['date'];
    $insert  = "update date_table SET date = '$date' where id= 1 ";
